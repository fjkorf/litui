...
//!
//! `define_markdown_app!` generates an `AppState` that, for each
...
// bevy 0.19 changed the Resource trait to `pub trait Resource: Component {}`.
// The macro-generated `Page`/`AppState` are plain types (not Components), so we
// can't `impl Resource` for them directly. Wrap them in newtypes that derive
// `Resource` (the derive also supplies the required `Component` impl). The
// `[custom]` slots live on the inner `AppState`, reached via `.0`; the
// take/replace closure-invocation pattern is unchanged by the wrapping.
