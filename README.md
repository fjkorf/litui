...
**Styled text from YAML frontmatter** — colors, sizes, backgrounds, all resolved at compile time:

![Style gallery](tests/snapshot_tests/snapshots/demo_styles.png)

**Interactive widgets embedded in markdown** — sliders, checkboxes, text inputs:

![Form widgets](tests/snapshot_tests/snapshots/demo_form.png)

**Cross-page state monitoring** — display widgets read shared `AppState` via bevy_ecs:

![State monitor](tests/snapshot_tests/snapshots/demo_monitor_with_values.png)

**Full markdown rendering** — headings, lists, blockquotes, code blocks, tables, links:
...
// Generates: Page enum, AppState, render functions, MdApp struct
```
...
| # | Tutorial | Example |
|---|----------|---------|
| 01 | [Hello Markdown](_tutorial::_01_hello_markdown) | `markdown_macro_example` |
| 02 | [Frontmatter Styles](_tutorial::_02_frontmatter_styles) | `widgets_example` |
| 03 | [Tables](_tutorial::_03_tables) | `eframe_demo` |
| 04 | [Widgets](_tutorial::_04_widgets) | `widgets_example` |
| 05 | [Selectors and Spans](_tutorial::_05_selectors_and_spans) | `selectors_example` |
| 06 | [Styled Containers](_tutorial::_06_styled_containers) | `selectors_example` |
| 07 | [Multi-Page Apps](_tutorial::_07_multi_page_apps) | `eframe_demo` |
| 08 | [Images](_tutorial::_08_images) | `markdown_macro_example` |
| 09 | [Third-Party Widgets](_tutorial::_09_third_party_widgets) | `third_party_widgets` |
| 10 | [Bevy Integration](_tutorial::_10_bevy_integration) | `bevy_demo` |
| 11 | [Game UI Patterns](_tutorial::_11_game_ui_patterns) | `game_demo` |
...
| **YAML frontmatter styles** | `::key` suffix resolved at compile time into `RichText` |
...
**Style properties:** `bold`, `italic`, `strikethrough`, `underline`, `color` (`"#RRGGBB"`), `background`, `size`, `monospace`, `weak`.
...
│   ├── markdown_to_egui_macro/      # Proc-macro: .md → egui TokenStream
│   └── markdown_to_egui_helpers/    # Runtime rendering functions
├── examples/
│   ├── demo_content/                # Shared 7-page demo content + generated types
│   ├── eframe_demo/                 # Multi-page demo (eframe + bevy_ecs)
│   ├── bevy_demo/                   # Multi-page demo (full Bevy + bevy_egui)
│   ├── markdown_macro_example/      # Minimal static example
│   ├── widgets_example/             # Stateful widgets example
│   └── third_party_widgets/         # 3rd-party integration example
...
```sh
cargo run -p eframe_demo              # multi-page demo (eframe)
cargo run -p bevy_demo                # multi-page demo (Bevy)
cargo run -p markdown_macro_example   # minimal static content
cargo run -p third_party_widgets      # 3rd-party widget integration
