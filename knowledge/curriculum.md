...
| 08 | Multi-Page Apps | `define_markdown_app!`, `Page` enum, `AppState`, parent frontmatter, `panel:` containers (left/right/top/bottom/window), `show_all()` | `08_multi_page` | 07 |
...
### 01 — Hello Markdown
- **Introduces:** `include_markdown_ui!`, headings (H1-H3), paragraphs, **bold**, *italic*, ~~strikethrough~~, bullet lists, numbered lists, nested lists, blockquotes, fenced code blocks, `inline code`, [links](url), horizontal rules
...
### 06 — Widgets
- **Introduces:** `[slider](field)`, `[checkbox](field)`, `[textedit](field)`, `[button](label)`, `[display](field)`, `{config}` widget config syntax, `widgets:` frontmatter section (min, max, label, hint, format), state struct generation (`MdFormState`), click counters, display self-declaration
- **Assumes:** 01-05
...
### 08 — Multi-Page Apps
- **Introduces:** `define_markdown_app!` macro, `page:` frontmatter (name, label, default), `Page` enum, shared `AppState`, per-page `render_*()` functions, `MdApp` struct, `show_nav()`, `show_page()`, parent frontmatter (`parent:` keyword), `panel:` containers (left, right, top, bottom, window), `show_all()` auto-dispatch, window `open:` visibility control
- **Assumes:** 01-07
...
- Input widgets inside `::: foreach` — not yet implemented
