...
2. Add test function to `tests/snapshot_tests/tests/snapshots.rs`:
   ...
   #[test]
   fn my_feature() {
       snapshot_markdown("my_feature", include_litui_ui!("fixtures/my_feature.md"));
   }
   ```
...
#[test]
fn my_form() {
    let (render, mut state) = include_litui_ui!("fixtures/my_form.md");
...
| `foreach_checkbox.md` | Widgets inside foreach: checkbox, button, typed row fields |

### Testing `define_litui_app!` Pages
...
- **Display widgets** (`[display](field)`) cannot be tested via `include_litui_ui!` — they reference `state.field` which only exists in `define_litui_app!`'s `AppState`. Use `define_litui_app!` in a test module (see `tests/demo_pages.rs` and `tests/parent_body.rs`).
...
```sh
cargo test -p litui_macro dump_ -- --nocapture
```

Tests live in `crates/litui_macro/tests/dump_events.rs`. Current dump tests:
...
The test `crates/litui_macro/tests/deeply_nested.rs` verifies that macro expansion compiles for deeply nested markdown structures.

## Bevy Demo Testing

The Bevy example (`examples/11_bevy/`) renders litui content via `bevy_egui`. `egui_kittest` does not support Bevy's render pipeline, so content correctness is verified by the eframe-based snapshot tests.

**Current approach:** The Bevy example is verified by `cargo check -p tut_11_bevy` (compile test) and manual smoke testing (`cargo run -p tut_11_bevy`).
