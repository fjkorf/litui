...
11. **Undefined style keys/classes error at compile time** — This is intentional. A typo in `::prommo` or `.prommo` should fail the build, not silently render unstyled text. The error message includes the key/class name and points to the source file.
...
13. **Stateful widgets change the macro return type** — If ANY stateful widget is present, `include_litui_ui!` returns `(fn, LituiFormState)` instead of a closure. Destructure as `let (render, mut state) = ...`.
...
15. **Shared widget fields must have matching types** — In `define_litui_app!`, all widget fields merge into a single `AppState`. Two pages can declare the same field name if the types match (e.g., both `[slider](volume)` — shared `f64`). Conflicting types (e.g., `[slider](foo)` on one page, `[checkbox](foo)` on another) produce a compile error.
...
19. **Unknown frontmatter keys are compile errors** — All frontmatter structs use `deny_unknown_fields`. A typo like `defualt: true` or `colur: "#FF0000"` produces a clear serde error at compile time naming the unknown field.

20. **Non-URL, non-widget links are compile errors** — `[tables](files)` where `tables` is not a valid widget name and `files` is not a URL produces a compile error listing valid widget names. If you need a regular hyperlink, the URL must start with `http://`, `https://`, `mailto:`, `file://`, `#`, or `/`.

21. **Unused widget configs are compile errors** — Widget configs defined in frontmatter `widgets:` but never referenced by any widget via `{key}` produce a compile error. This catches typos in both directions. In `define_litui_app!`, configs from the parent `_app.md` are validated against usage across all child pages.

22. **`show_all()` always works** — `LituiApp::show_all(ctx)` generates a nav bar + central panel layout even when no pages have `panel:`. Previously it was only generated when at least one page used `panel:`.

23. **Error messages use `compile_error!` not `panic!`** — All validation errors in the proc macro produce clean `error[E0XXX]` messages with spans pointing to the relevant file path in the macro invocation, not ugly "proc macro panicked" messages.
