...
8. **egui is from crates.io** — Migration from fjkorf/egui fork to upstream egui 0.34 is complete. All `[patch.crates-io]` overrides have been removed. `eframe::App` still uses `fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame)` (the runner calls it), but **eframe 0.34 deprecated `App::update`** in favor of a required `App::ui(&mut self, &mut egui::Ui, &mut Frame)`. Examples keep their panel logic in `update` and add an empty `ui` + `#[allow(deprecated)]`. Use `CentralPanel::default().show(ctx, |ui| { ... })`.
...
21. **Unused widget configs are compile errors** — Widget configs defined in frontmatter `widgets:` but never referenced by any widget via `{key}` produce a compile error. This catches typos in both directions. In `define_markdown_app!`, configs from the parent `_app.md` are validated against usage across all child pages.

22. **`show_all()` always works** — `MdApp::show_all(ctx)` generates a nav bar + central panel layout even when no pages have `panel:`. Previously it was only generated when at least one page used `panel:`.
