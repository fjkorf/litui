...
5. `[if](X)` → `::: if X`, `[/if]()` → `:::`
6. `::frame` line suffix → `::: frame stylename ... :::` block directive

Widget configs like `[slider](vol){cfg}` need NO changes.

## New Features (no migration needed)

### Runtime paragraph styles (`::$field`)

```markdown
Status message. ::$status_style
```

Auto-declares `status_style: String` on `AppState`. Set to a style name at runtime.
...
Auto-declares `show_inventory: bool` with X close button.
