...
- **`markdown_to_egui_macro`** (`crates/markdown_to_egui_macro/`) — Proc-macro crate. Exports `include_markdown_ui!()` and `define_markdown_app!()`.
  - `src/lib.rs` — Entry points, `load_and_parse_md`
  - `src/frontmatter.rs` — Types, YAML parsing, style merging, selector parsing
  - `src/parse.rs` — Fragment enum, `markdown_to_egui` event loop
  - `src/codegen.rs` — Token generation, `define_markdown_app_impl`, `AppInput`
- **`markdown_to_egui_helpers`** (`crates/markdown_to_egui_helpers/`) — Runtime rendering functions the macro-generated code calls.

### Examples

- **`demo_content`** (`examples/demo_content/`) — Shared 7-page demo content library. Contains `define_markdown_app!` invocation, `Page`/`AppState` types, render functions, and `auto_unmute` system. Used by both `eframe_demo` and `bevy_demo`.
- **`eframe_demo`** (`examples/eframe_demo/`) — eframe multi-page demo with bevy_ecs state management.
- **`bevy_demo`** (`examples/bevy_demo/`) — Full Bevy multi-page demo via bevy_egui.
- **`markdown_macro_example`** (`examples/markdown_macro_example/`) — Minimal single-file example.
- **`widgets_example`** (`examples/widgets_example/`) — Stateful widgets with hand-coded UI.
- **`third_party_widgets`** (`examples/third_party_widgets/`) — 3rd-party widget integration patterns.
...
eframe_demo -> demo_content -> { litui, bevy_ecs, egui }
bevy_demo -> demo_content + { bevy, bevy_egui }
...
cargo run -p eframe_demo                                   # run the multi-page demo (eframe)
cargo run -p bevy_demo                                     # run the multi-page demo (Bevy)
cargo run -p markdown_macro_example                        # run the minimal example
...
cargo test -p markdown_to_egui_macro dump_ -- --nocapture  # dump event streams
